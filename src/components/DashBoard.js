        ...
        <Table>
          ...
          <TableBody>
            <TransitionGroup component={null}>
              {rows.map((row) => (
                <CSSTransition nodeRef={nodeRef} in={inProp} key={row.id} timeout={500} classNames="fade">
                  <TableRow className='tablebody'>
                    ...
                    <TableCell /* style={{ color: row.change24h.startsWith('+') ? 'green' : 'red' }} */>{row.oprofit}</TableCell>
                    <TableCell>{row.avgdprofit}</TableCell>
                    <TableCell>{row.winpday}</TableCell>
                    <TableCell>{row.price ? row.price : '-'}</TableCell>

                    <TableCell>
                      <button style={{ backgroundColor: 'rgb(20, 8, 115, 0.9)', color: 'white', borderRadius: 4, height: '30px', width: '60px', border: 'none', padding: '4px 8px', cursor: 'pointer' }}>
                        {row.action}
                      </button>
                    </TableCell>
                  </TableRow>
